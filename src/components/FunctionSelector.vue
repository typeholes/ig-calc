<script setup lang="ts">
   ...
   import { libraryDescriptions, libraries } from '../js/libraryValues';
   ...
   function getFns(libraryName: string) {
   ...
   <div class="selector">
      ...
      <div class="cols" v-if="state.selecting">
         <div
            class="rows"
            :key="name"
            v-for="[name, description] of libraryDescriptions"
         >
         ...
         <template v-if="defined(state.libraryName)">
            <div class="rows">
               <div
                  :key="name"
                  v-for="([expr, description], name) of getFns(
                     state.libraryName
                  )"
               >
                  <div
                     class="rows nameDescription"
                     @click="selectFunction(name, expr.expr)"
                  >
                     <span>{{ name }}</span
                     ><span> {{ description }}</span>
                  </div>
