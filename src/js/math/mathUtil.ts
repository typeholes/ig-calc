...
export function freeVars(
   ...
   const free = node.filter(
      (x) =>
         isSymbolNode(x) && !hasProp(env, x.name) && !knownSymbols.has(x.name)
   ) as M.SymbolNode[];
...
export function getJavaScript(node: MathNode): string {
   ...
   if (isFunctionNode(node)) {
      ...
      const jsName = Math[name] instanceof Function ? 'Math.' + name : name;
      const result =
         jsName + '(' + node.args.map(getJavaScript).join(', ') + ')';
      return result;
   }
